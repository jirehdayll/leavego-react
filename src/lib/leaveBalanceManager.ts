...
/**
 * Local/offline fallback balance deduction.
 * Confirmed rule: each leave type deducts only its own balance.
 */
export function decreaseLeaveBalance(
  ...
  if (leaveTypeLower.includes('forced')) {
    ...
    balances.special_leave_privileges = Math.max(0, balances.special_leave_privileges - numDays);
  ...
  // Confirmed rule: no shared deduction. FL/SPL/WL do not touch Vacation/Sick.
  persistBalances(accountId, balances);
}
...
export function convertDBBalanceToLeaveBalances(dbBalance: any): LeaveBalances {
  ...
  // Already in LeaveBalances format.
  if (typeof dbBalance.forced_leave === 'number' && typeof dbBalance.wellness_leave === 'number') {
    return dbBalance as LeaveBalances;
  }

  // RPC summary format from get_user_leave_balance_summary():
  // { forced_leave: { balance }, special_leave: { balance }, ... }
  if (dbBalance.forced_leave && typeof dbBalance.forced_leave === 'object') {
    return {
      forced_leave: toNumber(dbBalance.forced_leave?.balance, 5),
      special_leave_privileges: toNumber(dbBalance.special_leave?.balance, 3),
      wellness_leave: toNumber(dbBalance.wellness_leave?.balance, 5),
      accumulated_sick: toNumber(dbBalance.sick_leave?.balance, 10),
      accumulated_vacation: toNumber(dbBalance.vacation_leave?.balance, 10),
      last_accumulation_date:
        dbBalance.sick_leave?.last_accrual ||
        dbBalance.vacation_leave?.last_accrual ||
        dbBalance.last_accrual_date ||
        dbBalance.last_accumulation_date,
    };
  }

  // Database row format from user_leave_balances.
  return {
