...
export default function LeaveForm() {
  ...
  const nextWorkingDate = (date) => {
    const d = new Date(date);
    while (d.getDay() === 0 || d.getDay() === 6) d.setDate(d.getDate() + 1);
    return d;
  };

  const prevWorkingDate = (date) => {
    const d = new Date(date);
    while (d.getDay() === 0 || d.getDay() === 6) d.setDate(d.getDate() - 1);
    return d;
  };
  ...
  // Compute and enforce max end date and auto-adjust weekends when applicable
  const [maxEndDate, setMaxEndDate] = useState('');

  useEffect(() => {
    const { start_date, leave_type } = formData;
    ...
    if (limit.unit === 'Calendar Days') {
      const maxDate = new Date(start);
      maxDate.setDate(maxDate.getDate() + (limit.max - 1));
      setMaxEndDate(maxDate.toISOString().split('T')[0]);
      return;
    }

    // For Working Days limit, walk forward counting working days
    let count = 0;
    ...
    setMaxEndDate(cur.toISOString().split('T')[0]);
  }, [formData.start_date, formData.leave_type]);

  useEffect(() => {
    const { start_date, end_date, leave_type } = formData;
    if (start_date && end_date && leave_type) {
      // Auto-adjust weekends for working-day-only leave types
      const excludeWeekends = 
      ...
      if (excludeWeekends && start && isWeekend(start)) {
        const next = nextWorkingDate(start);
        start = next.toISOString().split('T')[0];
        adjusted = true;
      }

      if (excludeWeekends && end && isWeekend(end)) {
        const prev = prevWorkingDate(end);
        end = prev.toISOString().split('T')[0];
        adjusted = true;
      }

      if (adjusted) {
        setFormData(prev => ({ ...prev, start_date: start, end_date: end }));
        // we'll let the next effect run to recalc days/validation
        return;
      }

      const calculatedDaysStr = calculateLeaveDuration(start, end, leave_type);
      ...
      // Enforce maxEndDate (if present)
      if (maxEndDate) {
        const ed = new Date(end);
        const max = new Date(maxEndDate);
        if (ed > max) {
          setValidationError(`Selected end date exceeds the allowed maximum for this leave type (${maxEndDate}).`);
        }
      }
    } else {
      setValidationError('');
    }
  }, [formData.start_date, formData.end_date, formData.leave_type, maxEndDate]);
