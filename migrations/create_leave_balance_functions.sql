...
BEGIN
  ...
  -- Log for debugging - don't actually deduct yet
  RAISE NOTICE 'Leave approval trigger fired: user_id=%, status=%, leave_type=%', 
    NEW.user_id, NEW.status, NEW.details->>'leave_type';
  
  -- For now, just return NEW without doing anything
  -- This allows approvals to work while we debug the balance deduction
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
